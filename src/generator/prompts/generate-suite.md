...
1. **Page Object Model is mandatory (when `pattern: 'pom'`).** This prompt is invoked specifically when the project's `reactlens.config.ts` sets `pattern: 'pom'` (the default). Every page gets a class in `e2e/pages/` whose constructor accepts a `Page` and exposes locators + actions. Specs ONLY interact with the page through the POM. The alternative pattern (`pattern: 'component-object'`) is generated by a different prompt and is out of scope here.
